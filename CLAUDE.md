...
9. **多人／多 agent 協作前，先看 Skill 分工。** 本專案搭配 5 個全域 Claude Code Skill：`itinerary-collab-router`（任何碰到 index.html/detail.html/support.js/行程資料.txt/協作指南.md/CLAUDE.md/.github/workflows/deploy.yml 的任務都先讀這個，決定還要不要接著讀其他 4 個）、`itinerary-content-schema`（新增/修改 `lm.*`/`day.*` 資料欄位時必讀，即使任務描述聽起來只是「調整版面」）、`itinerary-site-design-system`（純視覺/版面調整）、`itinerary-multi-page-architecture`（新增獨立頁面、多頁共用資料時）、`itinerary-deploy-release`（部署設定、API 金鑰、commit/push 前）。曾發生過：一個 session 以「調整 detail.html 版面」為由新增了 `highlight`/`tags`/`quickFacts` 三個欄位並在 UI 端渲染，卻沒同步定義 txt 語法、沒更新協作指南.md，導致欄位當下無法真正被編輯（見 `異動紀錄.log`）——這套 skill 分工＋`scripts/check-schema-doc-sync.js`（機械比對 `detail.html` 用到的欄位跟協作指南.md 有沒有文件化）就是為了防止這個情況再發生，同步/改動後可以跟 Node 語法檢查一起順手跑一次。
